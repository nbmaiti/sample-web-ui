...
describe('Test Profile Page Delete', () => {
  ...
  // Filter out WiFi profiles that may not be created (if wireless configs not available on real server)
  const profilesToDelete = amtProfiles.filter((profile: any) => {
    if (profile.profileName && profile.profileName.includes('-WiFi')) {
      return false
    }
    return true
  })
